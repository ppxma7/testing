function obj=stat_boxplot2(obj,varargin)
...
my_addParameter(p,'drawoutlier',1);
...
hndl=patch(xpatch,...
    ypatch,...
    [1 1 1],'FaceColor',draw_data.color,'EdgeColor','k','FaceAlpha',params.alpha,'EdgeAlpha',1,'LineWidth',params.linewidth);

obj.results.stat_boxplot{obj.result_ind,1}.box_handle=hndl;

%Draw medians
if params.notch
    obj.results.stat_boxplot{obj.result_ind,1}.median_handle=line([notchleft' ; notchright'],[p(:,4)' ; p(:,4)'],'Color','k','LineWidth',params.linewidth);
else
    obj.results.stat_boxplot{obj.result_ind,1}.median_handle=line([boxleft' ; boxright'],[p(:,3)' ; p(:,3)'],'Color','k','LineWidth',params.linewidth);
end

%Draw whiskers
obj.results.stat_boxplot{obj.result_ind,1}.lower_whisker_handle=line([boxmid' ; boxmid'],[p(:,1)' ; p(:,2)'],'Color','k','LineWidth',params.linewidth);
obj.results.stat_boxplot{obj.result_ind,1}.upper_whisker_handle=line([boxmid' ; boxmid'],[p(:,end-1)' ; p(:,end)'],'Color','k','LineWidth',params.linewidth);

%Draw outliers
if params.drawoutlier==1
...
% else
%     obj.results.stat_boxplot{obj.result_ind,1}.outliers_handle=plot(boxmid(outliersx),outliersy,'*','MarkerFaceColor',draw_data.color);
end
